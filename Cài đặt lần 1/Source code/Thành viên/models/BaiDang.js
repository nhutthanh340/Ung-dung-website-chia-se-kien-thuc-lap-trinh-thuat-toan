...
exports.readDetailBlogById = async function (id) {
    const sql = 'select BV.tomtat, ND.hoten, TL.tentheloai, BD.id, BD.ngaydang, BV.tenbaiviet, BV.noidung from nguoidung ND, theloaibaiviet TL, baidang BD, baiviet BV where ND.id=BV.idnguoigui and BV.id=BD.idbaiviet and TL.id=BV.idtheloaibaiviet and BD.id = ?';
    const result = await Connection.promise().query(sql, id);
    return result[0];
};
